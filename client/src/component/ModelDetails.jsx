...
function ModelDetails({ el, setModelOn, modelOn }) {
  ...
  const imgList = el.model_image
  const descList = el.model_desc
  //----Modal appear-------------------------------------//
      ...
      <div className='relative flex flex-col lg:flex-row items-center justify-center w-full h-full overflow-y-scroll lg:overflow-hidden pb-10 lg:pb-0'>
        ...
        <div className='lg:w-1/2 flex flex-col md:flex-row  md:space-x-10 lg:space-x-0 lg:flex-col items-center mt-2 md:mt-8 lg:mt-0'>
          ...
          <div ref={firstSection} className='relative text-slate-100 w-64 h-64 mt-4 lg:mt-8 flex justify-center items-center'>
            <div>
              <h2 className='font-bold text-3xl capitalize'>{el.model_name}</h2>
              <ul className='mt-2 ml-4 tracking-wider list-disc'>
              {descList.map((el,i) => {
            ...
            <div className='absolute bottom-0 flex items-center w-60'>
              ...
              <hr className='w-full' />
              <span className='absolute top-4 left-24 rockText'>{el.width} cm</span>
              <span>&#10146;</span>
            </div>
            ...
            <div className='absolute bottom-1/2 right-1/2  flex items-center w-60 rotate-90'>
              <span className='rotate-180'>&#10146;</span>
              <hr className='w-full' />
              <span className='absolute top-4 left-24 rockText'>{el.height} cm</span>
              <span>&#10146;</span>
            </div>
          </div>
        </div>
